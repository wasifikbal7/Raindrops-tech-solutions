...
const Navbar = ({ isTopOfPage, selectedPage, setSelectedPage }: Props) => {
  ...
  return (
    <nav>
      <div
        className={`${navbarBackground} ${flexBetween} fixed top-0 bg-stone-50 z-50 w-full backdrop-blur-sm shadow-md py-8`}
      >
        <div className={`${flexBetween} mx-auto w-5/6`}>
          <div className={`${flexBetween} w-full gap-20`}>
            {/* LEFT SIDE */}
            <div className="absolute navlogo left-8 w-72">
              <a href="#home">
      ...
      {!isAboveMediumScreens && isMenuToggled && (
        <div className="fixed right-0 bottom-0 z-50 bg-opacity-80 h-full backdrop-blur-sm w-[300px] bg-sky-100">
          ...
          {/* MENU ITEMS */}
          <div className="absolute mt-3 pt-4 ml-[25%] text-slate-800  h-[400px] flex items-center flex-col gap-9 justify-between">
            <div
              className={` flex flex-col gap-9 font-semibold text-xl ${flexBetween}`}
            >
              <Link
                page="Home"
                selectedPage={selectedPage}
              ...
              <div className="relative flex flex-col items-center justify-center text-lg font-semibold transition-transform text-gray-50 active:scale-90 left-2">
              <ActionButton variant='outline' className="text-slate-800 text-lg font-bold border-2 border-black rounded-[65px] bg-transparent" setSelectedPage={setSelectedPage}>
                GET A QUOTE
              </ActionButton>
              </div>
          </div>
